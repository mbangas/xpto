...
function parseGedcomToJson(text) {
  ...
  for (const rec of recs) {
    ...
    if (om) {
      ...
      for (let i = 1; i < rec.lines.length; i++) {
        const { lev, rest } = rec.lines[i];
        if (lev === 1 && rest.startsWith('FILE')) { fileObj = { file: rest.replace(/^FILE\s*/, '').trim(), form: '' }; obje.files.push(fileObj); }
        if (lev === 2 && rest.startsWith('FORM') && fileObj) fileObj.form = rest.replace(/^FORM\s*/, '').trim();
