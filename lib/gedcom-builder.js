 ...
 * @param {{ rewriteFilePath?: (path: string) => string }} [options]
 * @returns {string} GEDCOM text (lines joined by '\n')
 */
function buildGedcomText(collections, options = {}) {
  const rewriteFile = options.rewriteFilePath || (f => f);
  const {
    individuals  = {},
    families     = {},
    multimedia   = {},
  ...
  Object.values(individuals).filter(i => !i.deletedAt).forEach(indi => {
    ...
    // notes may be a plain string (legacy data) or an array
    const indiNotes = Array.isArray(indi.notes) ? indi.notes : (indi.notes ? [indi.notes] : []);
    indiNotes.forEach(n => L.push(`1 NOTE ${n}`));
    ...
    (indi.multimediaRefs || []).forEach(mref => {
      ...
      (mm.files || []).forEach(f => {
        ...
        const file = f.file ? rewriteFile(f.file) : f.file;
        const title = f.title;
