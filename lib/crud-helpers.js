...
/* ═══════════════════════════════════════════════════════════════════
   Tree settings (key/value per tree)
   ═══════════════════════════════════════════════════════════════════ */

async function readTreeSettings(treeId) {
  if (!usePg()) {
    ensureDataDir();
    const fp = require('path').join(getDataDir(), 'settings.json');
    if (!fs.existsSync(fp)) return {};
    try { return JSON.parse(fs.readFileSync(fp, 'utf8')); } catch (_) { return {}; }
  }
  const { rows } = await _db().query(
    'SELECT key, value FROM tree_settings WHERE tree_id = $1',
    [treeId],
  );
  const result = {};
...
async function writeTreeSetting(treeId, key, value) {
  if (!usePg()) {
    ensureDataDir();
    const fp = require('path').join(getDataDir(), 'settings.json');
    let cur = {};
  ...
  await _db().query(
    ...
    [treeId, key, JSON.stringify(value)],
  );
}

async function writeTreeSettings(treeId, settings) {
  for (const [key, value] of Object.entries(settings)) {
    await writeTreeSetting(treeId, key, value);
  }
}

async function deleteTreeSettings(treeId) {
  if (!usePg()) {
    ensureDataDir();
    const fp = require('path').join(getDataDir(), 'settings.json');
  ...
  await _db().query('DELETE FROM tree_settings WHERE tree_id = $1', [treeId]);
}

/* ═══════════════════════════════════════════════════════════════════
   Tree history (append-log per tree)
   ═══════════════════════════════════════════════════════════════════ */

async function readTreeHistory(treeId) {
  if (!usePg()) {
    ensureDataDir();
    const fp = require('path').join(getDataDir(), 'history.json');
    if (!fs.existsSync(fp)) return [];
    try { return JSON.parse(fs.readFileSync(fp, 'utf8')); } catch (_) { return []; }
  }
  const { rows } = await _db().query(
    'SELECT data FROM tree_history WHERE tree_id = $1 ORDER BY created_at DESC LIMIT 500',
    [treeId],
  );
  return rows.map(r => r.data);
}

async function appendTreeHistory(treeId, entries) {
  ...
  if (!usePg()) {
    ...
    let cur = [];
    if (fs.existsSync(fp)) cur = JSON.parse(fs.readFileSync(fp, 'utf8'));
    cur = arr.concat(cur).slice(0, 500);
    fs.writeFileSync(fp, JSON.stringify(cur, null, 2), 'utf8');
    return;
  }
  for (const entry of arr) {
    await _db().query(
      'INSERT INTO tree_history (tree_id, data) VALUES ($1, $2)',
      [treeId, JSON.stringify(entry)],
    );
  }
}

async function clearTreeHistory(treeId) {
  if (!usePg()) {
    ensureDataDir();
    const fp = require('path').join(getDataDir(), 'history.json');
    fs.writeFileSync(fp, '[]', 'utf8');
  ...
  await _db().query('DELETE FROM tree_history WHERE tree_id = $1', [treeId]);
}
