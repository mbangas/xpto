...
describe('Individuals — CREATE (POST /api/individuals)', () => {
  test('creates a new individual and returns 201 with correct fields', async () => {
    const res = await request(app).post('/api/individuals').send(INDI_BASE);
  ...
  test('auto-generates a sequential id with prefix I', async () => {
    const res = await request(app).post('/api/individuals').send({ ...INDI_BASE, names: [{ given: 'Ana', surname: 'Costa' }], sex: 'F' });
    expect(res.status).toBe(201);
    expect(res.body.id).toMatch(/^I\d+$/);
  });

  test('accepts a custom id when provided', async () => {
    const res = await request(app).post('/api/individuals').send({ ...INDI_BASE, id: 'I_CUSTOM' });
    expect(res.status).toBe(201);
    expect(res.body.id).toBe('I_CUSTOM');
  });

  test('creates individual with sex=F', async () => {
    const res = await request(app).post('/api/individuals').send({ ...INDI_BASE, sex: 'F', names: [{ given: 'Maria', surname: 'Sousa' }] });
    expect(res.status).toBe(201);
    expect(res.body.sex).toBe('F');
  });

  test('creates individual with default sex=U when not provided', async () => {
    const res = await request(app).post('/api/individuals').send({ names: [{ given: 'Pessoa', surname: 'Teste' }] });
    expect(res.status).toBe(201);
  ...
  test('stores multiple name parts (GIVN, SURN, NPFX, NSFX, NICK)', async () => {
    ...
    const res = await request(app).post('/api/individuals').send({ ...INDI_BASE, names: [richName] });
    expect(res.status).toBe(201);
...
describe('Individuals — READ (GET /api/individuals)', () => {
  ...
  test('GET /api/individuals returns an array', async () => {
    const res = await request(app).get('/api/individuals');
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
  });

  test('GET /api/individuals/:id returns the individual', async () => {
    const res = await request(app).get(`/api/individuals/${createdId}`);
    expect(res.status).toBe(200);
  ...
  test('GET /api/individuals/:id returns 404 for non-existent id', async () => {
    const res = await request(app).get('/api/individuals/XXXXXXXX');
    expect(res.status).toBe(404);
  });

  test('GET /api/individuals excludes soft-deleted records by default', async () => {
    // Create and delete an individual
    const post = await request(app).post('/api/individuals').send({ ...INDI_BASE, names: [{ given: 'Para Apagar' }] });
    const delId = post.body.id;
    await request(app).delete(`/api/individuals/${delId}`);

    const list = await request(app).get('/api/individuals');
  ...
  test('GET /api/individuals?includeDeleted=true includes soft-deleted records', async () => {
    const post = await request(app).post('/api/individuals').send({ ...INDI_BASE, names: [{ given: 'Deleted Indi' }] });
    const delId = post.body.id;
    await request(app).delete(`/api/individuals/${delId}`);

    const all = await request(app).get('/api/individuals?includeDeleted=true');
...
describe('Individuals — UPDATE (PUT /api/individuals/:id)', () => {
  ...
  test('updates an individual and returns the updated record', async () => {
    const res = await request(app).put(`/api/individuals/${indiId}`)
      .send({ names: [{ given: 'João Pedro', surname: 'Silva' }] });
    expect(res.status).toBe(200);
  ...
  test('updating sets a new updatedAt timestamp', async () => {
    const before = (await request(app).get(`/api/individuals/${indiId}`)).body.updatedAt;
    await new Promise(r => setTimeout(r, 10));
    const res = await request(app).put(`/api/individuals/${indiId}`).send({ sex: 'F' });
    expect(res.status).toBe(200);
    expect(res.body.updatedAt).not.toBe(before);
  });

  test('PUT on non-existent id returns 404', async () => {
    const res = await request(app).put('/api/individuals/XXNOTFOUND').send({ sex: 'M' });
    expect(res.status).toBe(404);
  });
});
...
describe('Individuals — DELETE (soft delete)', () => {
  test('DELETE sets deletedAt and returns { ok: true }', async () => {
    const post = await request(app).post('/api/individuals').send(INDI_BASE);
    const id   = post.body.id;

    const del = await request(app).delete(`/api/individuals/${id}`);
    expect(del.status).toBe(200);
    expect(del.body.ok).toBe(true);

    // Record still exists but has deletedAt set
    const full = await request(app).get(`/api/individuals/${id}`);
    expect(full.status).toBe(200);
    expect(full.body.deletedAt).toBeTruthy();
  });

  test('DELETE on non-existent id returns 404', async () => {
    const res = await request(app).delete('/api/individuals/NOEXIST');
    expect(res.status).toBe(404);
  });
});
...
describe('Individuals — GEDCOM 7 structural compliance', () => {
  ...
  test.each(['M', 'F', 'X', 'U'])('accepts GEDCOM 7 sex value: %s', async (sex) => {
    const res = await request(app).post('/api/individuals').send({ ...INDI_BASE, sex });
    expect(res.status).toBe(201);
