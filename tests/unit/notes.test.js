...
describe('Notes — CREATE', () => {
  test('creates a note and returns 201', async () => {
    const res = await request(app).post('/api/notes').set(AUTH).send(NOTE_BASE);
  ...
  test('creates an empty note (text defaults to empty string)', async () => {
    const res = await request(app).post('/api/notes').set(AUTH).send({});
    expect(res.status).toBe(201);
...
describe('Notes — READ', () => {
  ...
  test('GET /api/notes/:id returns the note', async () => {
    const res = await request(app).get(`/api/notes/${noteId}`).set(AUTH);
    expect(res.status).toBe(200);
  ...
  test('GET on non-existent id returns 404', async () => {
    const res = await request(app).get('/api/notes/NNOPE').set(AUTH);
...
describe('Notes — UPDATE', () => {
  ...
  test('updates the text of a note', async () => {
    const res = await request(app).put(`/api/notes/${noteId}`)
      .set(AUTH).send({ text: 'Texto da nota actualizado.' });
    expect(res.status).toBe(200);
    expect(res.body.text).toBe('Texto da nota actualizado.');
  });

  test('PUT on missing id returns 404', async () => {
    const res = await request(app).put('/api/notes/NNOPE').set(AUTH).send({ text: 'X' });
    expect(res.status).toBe(404);
  });
});
...
describe('Notes — DELETE', () => {
  test('DELETE soft-deletes a note', async () => {
    const post = await request(app).post('/api/notes').set(AUTH).send({ text: 'A apagar' });
    const id   = post.body.id;

    const del = await request(app).delete(`/api/notes/${id}`).set(AUTH);
    expect(del.status).toBe(200);
    expect(del.body.ok).toBe(true);

    const rec = (await request(app).get(`/api/notes/${id}`).set(AUTH)).body;
...
describe('Notes — GEDCOM 7 compliance', () => {
  ...
  test('note text is stored as a plain UTF-8 string', async () => {
    ...
    const res = await request(app).post('/api/notes').set(AUTH).send({ text: unicodeText });
    expect(res.status).toBe(201);
